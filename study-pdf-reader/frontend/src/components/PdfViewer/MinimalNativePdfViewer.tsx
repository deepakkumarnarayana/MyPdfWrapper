...
export const MinimalNativePdfViewer: React.FC = () => {
  ...
  // Direct PDF path and minimal PDF.js viewer
  const pdfPath = `/sample-pdfs/${bookPdfMap[bookId]}`;
  const viewerUrl = `/pdfjs-minimal/viewer.html?file=${encodeURIComponent(pdfPath)}`;
    ...
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#525659' }}>
      ...
      <Box sx={{ 
        ...
        <Typography variant="h6" sx={{ color: 'white' }}>
          {bookPdfMap[bookId]} - Complete PDF.js Viewer
        </Typography>
      </Box>

      {/* Minimal PDF.js viewer (only 5 files!) */}
      <Box sx={{ flexGrow: 1 }}>
        <iframe
          src={viewerUrl}
          style={{
